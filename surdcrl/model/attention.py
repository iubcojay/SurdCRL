...
class TrajectoryAttention(nn.Module):
    ...
    def forward_framewise(self, x,thw_prev):  # x: [bs*T, P, C]
        BxT, N, C = x.shape
        h = self.num_heads
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x,thw_prev  # [bs*T, P, C]
    def forward(self, x, thw_prev, with_cls_token=False):
        B, N, C = x.shape # BS,1600,768
        # 200:uniformerv2  2081:internvideo2
        if N == 200 or N == 2081:
            return self.forward_framewise(x,thw_prev)
        else:
