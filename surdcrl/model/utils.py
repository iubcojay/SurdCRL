...
class ObjectsCrops(nn.Module):
    ...
    def forward(self, features, boxes):
        ...
        PyTorch 的 roi_align 底层依赖 CUDA 实现，默认情况下可能启用非确定性算法。
        即使设置 torch.backends.cudnn.deterministic=True，某些版本的 roi_align 仍无法完全保证确定性。
        所以把roi_align移动到cpu上计算
        """
        BS, d, T, H, W = features.shape
        O = boxes.size(2)
        output_size = (H, W)
        spatial_scale = [H/224, W/224][0]
        ...
        # features：torch.Size([16, 768, 14, 14])   
        # boxes：torch.Size([16, 4, 4])
        # features_cpu = features.cpu()
        # boxes_cpu = boxes.cpu()
        ret = roi_align(
            features,
            list(torch.unbind(boxes.float(), dim=0)),
            output_size,
            spatial_scale,
            self.sampling_ratio,
            self.aligned,
        ...
        # ret = ret.to(features.device)  # 移回 GPU
        # T = T // 2
