...
class InternVideo2(nn.Module):
    ...
    def forward(self, x,box):
        ...
        # # 对象时序变化消息传递模块
        ym,yt = self.vfcm(objx)
        yt = yt.reshape(L*(T-1), BS*O, C)
...
class orcausal(nn.Module):
    def __init__(self, num_classes=20, feature_dim: int = 768, num_objects: int = 3):
        super().__init__()
        self.moduleobj = internvideo2_base_patch14_224(pretrained=True,num_classes=num_classes, num_objects=num_objects)
        self.modulecontext = internvideo2_base_patch14_224_context(pretrained=True,num_classes=20)
        ...
        # trunc_normal_(self.classifer.weight, std=.02)
        # self.classifer.weight.data.mul_(0)
        # self.classifer.bias.data.mul_(0)
...
#         ipdb.set_trace()
    ...
    def forward(self,x,metadata: torch.Tensor,back):
        obj = self.moduleobj(x,metadata)
        context,_ = self.modulecontext(x)
...
if __name__ == '__main__':
    ...
    model = orcausal(num_classes=20)
    # model = internvideo2_large_patch14_224(num_classes=400).cuda().half()
    # output = model(torch.rand(1, 3, num_frames, img_size, img_size),box,torch.rand(1, 3, num_frames, img_size, img_size))
    # model = internvideo2_base_patch14_224(pretrained=True,num_classes=20).cuda().half()
    # print(model)

    gflops, params = profile(model, inputs=(torch.rand(1, 3, num_frames, img_size, img_size),box,torch.rand(1, 3, num_frames, img_size, img_size)))
    gflops = gflops / 1e9
